...
pfluid = 0.562/0.63 * 10^-12 #kg/m^3
...
```{r, echo=FALSE}
#Calculation of density from concentration 
weight_actin = 42000 #Da

M_weight_actin = 42000*6.02214076*10^23 #Da

#1 Da = 1.6605e-24 gramm thus MW_act in gramm is
MW_act = M_weight_actin*1.6605*10^-24 #gramm

#concentrations
concentrations <- as.numeric(Exp2$`Konzentration [µM]`)

#want density in kg/m^3  c is in 10^-6 Mol/ l 
density <- concentrations*10^-6*10^3* MW_act 
...
#formula for calculatiung viscosity:
visc_exp2<- ((5*pi/42*K*dsphere^2*density*g*sin(theta2*pi/180))/v2 *(psphere-density)/density*(dtube+dsphere)/dsphere)
...
kable(visc_df2, digits = c(1,50,50))
 
kable(visc_plot_data2, digits = c(1,50,50))
